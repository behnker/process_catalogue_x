"""Process-to-System linkage endpoints (from process side)."""
...
@router.get("/{process_id}/systems", response_model=ProcessSystemsResponse)
async def get_process_systems(
    process_id: str,
    user: CurrentUser = Depends(get_current_user),
    db: AsyncSession = Depends(get_tenant_db),
):
    """Get all systems linked to a process."""
...
async def link_system_to_process(
    process_id: str,
    body: ProcessSystemCreate,
    user: CurrentUser = Depends(get_current_user),
    db: AsyncSession = Depends(get_tenant_db),
):
    """Link a system to a process (from process side)."""
    # Verify process exists
    proc_result = await db.execute(
        select(Process).where(
            Process.id == process_id,
    ...
    sys_result = await db.execute(
        select(SystemCatalogue).where(
            SystemCatalogue.id == body.system_id,
    ...
    existing = await db.execute(
        select(ProcessSystem).where(
            ProcessSystem.organization_id == user.organization_id,
            ProcessSystem.process_id == process_id,
            ProcessSystem.system_id == body.system_id,
        )
    )
...
@router.delete("/{process_id}/systems/{link_id}", status_code=status.HTTP_204_NO_CONTENT)
async def remove_process_system_link(
    process_id: str,
    link_id: str,
    user: CurrentUser = Depends(get_current_user),
    db: AsyncSession = Depends(get_tenant_db),
